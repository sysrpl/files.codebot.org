...
class FileManager {
    ...
    entry(file: File, contentType: string): FileEntry {
        return {
            key: this.key(file),
            name: file.name,
            size: file.size,
    ...
    contentType(filename: string, callback: Action<string>) {
        let data = new FormData();
        data.append("filename", filename);
        postRequest(this.endpoint + "&action=contenttype", data, request => {
     ...
     */
    upload(file: File, contentType: string, onprogress?: FileProgressCallback, onerror?: Proc): FileEntry {
        ...
        let entry = this.entry(file, contentType);
